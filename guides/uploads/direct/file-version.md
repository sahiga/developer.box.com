...
the [upload][upload]`POST /files/:id/content`[upload][upload] API with the content of the file, the
...
rejected it is recommended to perform a [preflight][preflight] before
uploading the file.
...
to the [upload][upload]. These parameters
...
For file versions an additional [consistency][consistency]`If-Match` header header[consistency][consistency] can be passed
...
please use the [chunked][chunked].
